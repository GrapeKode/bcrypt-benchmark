...
const bcryptSync = (() => {
  console.log(`START: ${maxIterations} iterations for sync <bcryptjs> with saltRound ${saltRounds}`)
  const startDate = new Date()
  test(maxIterations > 0)
  const endDate = new Date()
  console.log(`END: Execution for sync <bcryptjs> with saltRound ${saltRounds} ended at: ${(endDate - startDate)/1000}sec`)
