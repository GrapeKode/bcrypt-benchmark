...
const bcryptjsAsync = (async () => {
  console.log(`START: ${maxIterations} iteration for async <bcryptjs> with saltRound ${saltRounds}`)
  const startDate = new Date()
  await test(maxIterations > 0)
  const endDate = new Date()
  console.log(`END: Execution for async <bcryptjs> with saltRound ${saltRounds} ended at: ${(endDate - startDate)/1000}sec`)
